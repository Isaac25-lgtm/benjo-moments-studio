...
@admin.route('/income/delete/<int:id>')
@login_required
...
@admin.route('/expenses/delete/<int:id>')
@login_required
...
def add_customer():
    ...
    if not all([name, service, total_amount]):
        flash('Name, service, and total amount are required.', 'error')
    else:
...
@admin.route('/customers/delete/<int:id>')
@login_required
...
def add_invoice():
    ...
    if not all([invoice_number, customer_id, date, amount]):
        flash('All fields are required.', 'error')
    else:
...
@admin.route('/invoices/mark-paid/<int:id>')
@login_required
...
@admin.route('/invoices/delete/<int:id>')
@login_required
...
def add_asset():
    ...
    if not all([name, category, value]):
        flash('Name, category, and value are required.', 'error')
    else:
...
@admin.route('/assets/delete/<int:id>')
@login_required
def delete_asset(id):
    """Delete asset."""
...
@admin.route('/gallery/toggle/<int:id>')
@login_required
def toggle_image(id):
...
def website_settings():
    ...
    return render_template('admin/website_settings.html', settings=settings)
...
def add_pricing():
    ...
    if request.method == 'POST':
        ...
        database.add_pricing_package(name, description, price, price_label, icon, features, is_featured, display_order)
...
def edit_pricing(id):
    ...
    if request.method == 'POST':
        ...
        database.update_pricing_package(id, name, description, price, price_label, icon, features, is_featured, display_order)
        flash('Pricing package updated successfully!', 'success')
        return redirect(url_for('admin.pricing'))
    
    return render_template('admin/pricing_form.html', package=package)

@admin.route('/pricing/delete/<int:id>')
@login_required
...
@admin.route('/pricing/toggle/<int:id>')
@login_required
